...
def is_cjk(char):
    ...
    if 0xFF00 <= code <= 0xFFEF:
        return True
    # General CJK-adjacent quote punctuation.
    return bool(0x2000 <= code <= 0x206F and char in "“”‘’")
...
class Terminal:
    ...
    def render(self, show_cursor=False, cursor_frame=0, duration=60):
        ...
        if show_cursor and cursor_frame % 6 < 4:
...
def show_spinner(
    ...
    if success_text is not None:
        final_line = [(success_prefix, success_color)] + (
            list(success_text) if isinstance(success_text, list) else [(success_text, TEXT_COLOR)]
        )
...
def generate_demo():
    ...
    prompt = [("⚡ ", PROMPT_COLOR)]
    type_text(
        terminal, prompt, "我想学习MoE。用scholar-agent帮我找一些文章到本地并做笔记", USER_COLOR, pause_end_ms=1000
    )
    ...
    show_spinner(
        ...
        success_text=[
            (
                "I will search for papers on Mixture of Experts (MoE), download the SOTA paper, and analyze it.",
                TEXT_COLOR,
            )
        ],
    ...
    show_spinner(
        terminal,
        [
            ("Calling tool: mcp__scholar-agent__search_papers", ACCENT_COLOR),
            ('(query="mixture of experts", top_n=3)', DIM_COLOR),
        ],
        duration_frames=8,
        success_prefix="✓ ",
    ...
    terminal.add_line(
        [
            (
                '  1. "Post Reasoning: Improving the Performance of Non-Thinking Models at No Cost" (2605.06165)',
                TEXT_COLOR,
            )
        ]
    )
    ...
    terminal.add_line(
        [('  3. "Outrageously Large Neural Networks: The Sparsely-Gated MoE Layer" (1701.06538)', DIM_COLOR)]
    )
    ...
    terminal.add_line(
        "Paper PDF downloaded to: paper-notes/large-language-models/Post_Reasoning_Improving_...pdf", DIM_COLOR
    )
    ...
    show_spinner(
        terminal,
        [
            ("Calling tool: mcp__scholar-agent__extract_paper_images", ACCENT_COLOR),
            ('(paper_id="2605.06165")', DIM_COLOR),
        ],
        duration_frames=8,
        success_prefix="✓ ",
    ...
    terminal.add_line(
        "Extracted figures to: paper-notes/large-language-models/Post_Reasoning_Improving_.../images/", DIM_COLOR
    )
    terminal.render(show_cursor=False, duration=600)
    ...
    terminal.add_line(
        "Generated draft analysis note at staging path: paper-notes/.staging/Post_Reasoning/note.md", DIM_COLOR
    )
    terminal.add_line(
        "Now, I will run the validation CLI script to ensure the generated note conforms to the quality gates.",
        TEXT_COLOR,
    )
    terminal.render(show_cursor=False, duration=1000)
    ...
    type_text(
        terminal,
        [("scholar-agent % ", DIM_COLOR)],
        "python scripts/validate_note.py --note paper-notes/.staging/Post_Reasoning/note.md",
        USER_COLOR,
        pause_end_ms=600,
    )
    ...
    terminal.add_line(
        '  "summary": { "has_frontmatter": true, "section_count": 8, "dataset_policy": "fallback" }', DIM_COLOR
    )
    terminal.add_line("}", DIM_COLOR)
    ...
    terminal.add_line(
        '  "target": "paper-notes/large-language-models/Post_Reasoning_Improving_the_Performance_of_Non-Thinking_Models_at_No_Cost/Post_Reasoning_Improving_the_Performance_of_Non-Thinking_Models_at_No_Cost.md"',
        SUCCESS,
    )
    terminal.add_line("}", DIM_COLOR)
    ...
    show_spinner(
        ...
        success_prefix="",
        success_text=[
            ("I will search the local knowledge base to retrieve the core findings of the paper.", TEXT_COLOR)
        ],
        success_color=TEXT_COLOR,
        success_pause_ms=800,
    )
    ...
    show_spinner(
        terminal,
        [
            ("Calling tool: mcp__scholar-agent__query_knowledge", ACCENT_COLOR),
            ('(query="Post-Reasoning 核心结论", limit=3)', DIM_COLOR),
        ],
        duration_frames=8,
        success_prefix="✓ ",
    ...
    terminal.add_line(
        [
            ("  1. 后置推理有效性：", HIGHLIGHT),
            ("让模型先输出答案再生成论证，同样可以显著提升模型准确率。", TEXT_COLOR),
        ],
        TEXT_COLOR,
    )
    terminal.add_line(
        [
            ("  2. 零推理开销：", HIGHLIGHT),
            ("最终答案的生成无需等待中间 Reasoning Trace，可在首包中截断返回，省去 10x token 成本。", TEXT_COLOR),
        ],
        TEXT_COLOR,
    )
    terminal.add_line(
        [
            ("  3. 强跨域泛化：", HIGHLIGHT),
            ("通过掩码损失微调（PR SFT），纯数学数据训练的模型也能提升科学、逻辑推理性能。", TEXT_COLOR),
        ],
        TEXT_COLOR,
    )
